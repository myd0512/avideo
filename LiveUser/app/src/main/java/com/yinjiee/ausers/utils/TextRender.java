...
import com.yinjiee.ausers.custom.VerticalImageSpan;
...
public class TextRender {
    ...
    private static SpannableStringBuilder createPrefix(Drawable levelDrawable, LiveChatBean bean) {
        ...
        if(levelDrawable==null){
            levelDrawable=ContextCompat.getDrawable(AppContext.sInstance,R.drawable.drawable_transparent);
        }
...
//        }
    ...
    public static void render(final TextView textView, final LiveChatBean bean) {
        ...
        if(bean.isAnchor()){
            levelBean = AppConfig.getInstance().getAnchorLevel(bean.getLevel());
        }else {
        ...
        ImgLoader.displayDrawable2(levelBean.getThumb(), new ImgLoader.DrawableCallback() {
            @Override
        ...
        ImgLoader.displayDrawable2(levelBean.getThumb(), new ImgLoader.DrawableCallback() {
            ...
            public void callback(Drawable drawable) {
                if (textView != null) {
                    ...
                    String name = bean.getUserNiceName() + " "+bean.getContent();
                    builder.append(name);
                    int end = start + name.length();
                    builder.setSpan(sWhiteColorSpan, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                    builder.setSpan(sFontSizeSpan4, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//                    builder.append(bean.getContent());
     ...
     * 渲染直播间用户弹窗数据
     */
    public static CharSequence renderLiveUserDialogData(String num) {
     ...
     * 生成系统+跟投前缀
     */
    public static SpannableStringBuilder createPrefixTel( LiveChatBean bean) {
        Drawable drawableSystem=ContextCompat.getDrawable(AppContext.sInstance,R.mipmap.photos22);
        SpannableStringBuilder builder = new SpannableStringBuilder();
        builder.append("  ");
        drawableSystem.setBounds(0, 0, DpUtil.dp2px(30), DpUtil.dp2px(17));
        builder.setSpan(new VerticalImageSpan(drawableSystem), 0, 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//        builder.setSpan(sFontSizeSpan4, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
     ...
     * 生成系统+跟投前缀
     */
    public static SpannableStringBuilder createPrefixSystem( LiveChatBean bean) {
        Drawable drawableSystem=ContextCompat.getDrawable(AppContext.sInstance,R.mipmap.photos22);
        SpannableStringBuilder builder = new SpannableStringBuilder();
        builder.append("  ");
        drawableSystem.setBounds(0, 0, DpUtil.dp2px(30), DpUtil.dp2px(17));
...
//        builder.setSpan(sFontSizeSpan4, start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        Drawable vipDrawable = ContextCompat.getDrawable(AppContext.sInstance, R.mipmap.photos23);
        if (vipDrawable != null) {
            builder.append("  ");
            vipDrawable.setBounds(0, 0, DpUtil.dp2px(39), DpUtil.dp2px(19));
            builder.setSpan(new VerticalImageSpan(vipDrawable), index, index + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            index += 2;
        }
        return builder;
