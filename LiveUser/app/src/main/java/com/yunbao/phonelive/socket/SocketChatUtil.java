...
public class SocketChatUtil {
     ...
     * 发系统消息
     */
    public static void sendSystemMessage(SocketClient client,String gameId,String itemIds, String content,int type) {
        if (client == null) {
            return;
        }
        UserBean u = AppConfig.getInstance().getUserBean();
        ...
        String systemType;
        if (type == 1){
            systemType = Constants.SOCKET_SYSTEM_ZJ;
        }else if(type ==2){
            systemType = Constants.SOCKET_SYSTEM_TZ;
            isTz = true ;
        }else {
            systemType = Constants.SOCKET_SYSTEM;
        }
        ...
        SocketSendBean sendBean = new SocketSendBean()
                ...
                .param("uname", u.getUserNiceName())
                .param("uid", u.getId())
                .param("ct", systemType+"#"+content) ;

        if(isTz && !TextUtils.isEmpty(gameId) && !TextUtils.isEmpty(itemIds)){
            sendBean.param("levelI",gameId) ;
            sendBean.param("guard_type",itemIds) ;
        }
     ...
     * 发系统消息
     */
    public static void sendSystemMessage(SocketClient client, String content,int type) {
        sendSystemMessage(client,null,null,content,type) ;
    }
