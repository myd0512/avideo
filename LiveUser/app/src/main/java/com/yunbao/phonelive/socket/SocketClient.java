            ...
            switch (map.getString("_method_")) {
                    ...
                    if (content.contains("#")) {
                        ...
                        switch (strArr[0]) {
                            ...
                            case Constants.SOCKET_SYSTEM_ZJ:
                                ...
                                //中奖消息是主播端推送的
                                //1、中奖信息格式 action=13, SystemNotZj#姓名#游戏名
                                //2、还有一个系统推的消息 action=90，SystemNotZj#中奖内容

                                String gameName = strArr[1] ;
                                if(strArr.length > 2){
                                    gameName = "恭喜" + gameName + "在" + strArr[2] + "中奖了" ;
                                }

                                liveChatBean = new LiveChatBean();
                                liveChatBean.setType(LiveChatBean.ZJ_LIST);
                                liveChatBean.setContent(gameName);
                ...
                case Constants.SOCKET_SEND_MSG://文字消息，点亮，用户进房间，这种混乱的设计是因为服务器端逻辑就是这样设计的,客户端无法自行修改
                    ...
                    if ("2".equals(msgtype)) {//发言，点亮
                        ...
                        Log.e(SocketClient.class.getSimpleName(), map.toJSONString());
