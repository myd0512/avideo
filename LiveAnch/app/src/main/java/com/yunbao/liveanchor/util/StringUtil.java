...
//    }
     ...
     * 把一个long类型的总毫秒数转成时长
     */
    public static String getDurationText(boolean defHour,long mms) {
    ...
    public static String getDurationText(long mms) {
        return getDurationText(false,mms) ;
    }
     ...
     * @return money
     */
    public static String converMoney2Point(String money,int num){
        if(TextUtils.isEmpty(money)){
            return "0" ;
        }
        ...
        BigDecimal moneyBid = new BigDecimal(money);
        return moneyBid.setScale(num, BigDecimal.ROUND_HALF_UP).toPlainString();
    }
