...
public class LiveRoomCheckLivePresenter {
    ...
    private final ThreadLocal<LiveBean> mLiveBean = new ThreadLocal<LiveBean>();
    ...
    public void roomCharge() {
        if (mLiveBean.get() == null) {
            return;
        }
        HttpUtil.roomCharge(mLiveBean.get().getUid(), mLiveBean.get().getStream(), mRoomChargeCallback);
    }
    ...
    private void enterLiveRoom() {
        if (mActionListener != null) {
            mActionListener.onLiveRoomChanged(mLiveBean.get(),mLiveType, mLiveTypeVal);
        }
    }
