...
public class SocketClient {
    ...
    private Emitter.Listener mTimeOutListener = new Emitter.Listener() {
        @Override
        public void call(Object... args) {
            L.e(TAG, "--onConnectTimeOut-->" + args.toString());
        }
    };
        ...
        private void processBroadcast(String socketMsg) {
            ...
            switch (map.getString("_method_")) {
                case Constants.SOCKET_SYSTEM://
                    ...
                    if (content.contains("#")) {
                        String[] strArr = map.getString("ct").split("#");
                        switch (strArr[0]) {
                            case Constants.SOCKET_SYSTEM_TZ:
                                LiveChatBean liveChatBean = new LiveChatBean();
                                liveChatBean.setType(LiveChatBean.TZ_FOLLOW);
                                liveChatBean.setContent(strArr[1]);
                                liveChatBean.setGameId(map.getString("levelI")) ;
                                liveChatBean.setGameItemIds(map.getString("guard_type")) ;
                                mListener.onTzFollow(liveChatBean);
                                break;
                            case Constants.SOCKET_SYSTEM_ZJ:

                                //中奖消息是主播端推送的
                                //1、中奖信息格式 action=13, SystemNotZj#中奖内容

                                liveChatBean = new LiveChatBean();
                                liveChatBean.setType(LiveChatBean.ZJ_LIST);
                                liveChatBean.setContent(strArr[1]);
                                mListener.onZjList(liveChatBean);
                                break;
                            default:
                                systemChatMessage(strArr[1]);
                                break;
                        }
                    }else{
                ...
                case Constants.SOCKET_SEND_MSG://文字消息，点亮，用户进房间，这种混乱的设计是因为服务器端逻辑就是这样设计的,客户端无法自行修改
                    ...
                    if ("2".equals(msgtype)) {//发言，点亮
                        ...
                        L.e(SocketClient.class.getSimpleName(), map.toJSONString());
...
//                    break;
    ...
    private void printMessage(Object info){
