...
import com.yunbao.liveanchor.util.GifCacheUtil;
...
public class LiveEnterRoomAnimPresenter {
    ...
    public LiveEnterRoomAnimPresenter(Context context, View root) {
        ...
        mLevel = root.findViewById(R.id.iv_vip_level);
...
//        mDp500 = DpUtil.dp2px(500);
        mDp500 = ScreenDimenUtil.getInstance().getScreenWdith() ;
    ...
    public void enterRoom(LiveEnterRoomBean bean) {
        int level=bean.getLiveChatBean().getLevel();
        //首先判断等级
        //等级大于3显示动画 根据等级更换动画背景
        if(level>=3&&level<=10){
            ImgLoader.display(R.mipmap.icon_vip_bg1,mBg);
            ImgLoader.display(R.mipmap.icon_vip_heade1,mLevel);
        }else if(level>10&&level<=15){
            ImgLoader.display(R.mipmap.icon_vip_bg2,mBg);
            ImgLoader.display(R.mipmap.icon_vip_heade2,mLevel);
        }else if(level>15){
            ImgLoader.display(R.mipmap.icon_vip_bg3,mBg);
            ImgLoader.display(R.mipmap.icon_vip_heade3,mLevel);
        }else {
            return;
        }
        if (mIsAnimating) {
